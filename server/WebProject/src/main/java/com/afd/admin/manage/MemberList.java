...
@WebServlet("/main/admin/manage/memberlist.do")
public class MemberList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (n == 1) {
			...
			pagebar += pagebar += String.format(
					" <li><a href='/webproject/main/admin/manage/memberlist.do?page=%d' aria-label='Previous'> <span aria-hidden='true'>&laquo;</span></a></li> ",
					n - 1);
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			if (n == nowPage) {
				...
				pagebar += String.format(" <li><a href='/webproject/main/admin/manage/memberlist.do?page=%d'>%d</a></li> ", n, n);
			}
		...
		ArrayList<MemberManageDTO> list = dao.list(map);
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/main/admin/manage/memberlist.jsp");
